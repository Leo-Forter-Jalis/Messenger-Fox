...
class ChatService(private val chatDAO: ChatDataTable, private val chatMemberDao: ChatMemberDataTable){
    fun createGroupChat(request: CreateGroupChatRequest, ownerId: UUID) : Response{
        return chatDAO.createGroupChat(request.chatName, request.chatTag).fold(onSuccess = {chat ->
            ...
            chatMemberDao.addMemberForChat(chat.chatId, ownerId, Role.OWNER).fold(onSuccess = { owner ->
    ...
    fun createPrivateChat(request: CreatePrivateChatRequest, userId: UUID) : Response{
        return chatDAO.createPrivateChat().fold(onSuccess = { chatB ->
            val list = chatMemberDao.addMembersForChat(chatB.chatId, listOf(request.interlocutor, userId), Role.MEMBER)
    ...
    fun selectUsersForChatId(request: ChatMembersInChatRequest) : List<ChatMember> {
        return chatMemberDao.selectUserByChatId(request.chatId)
    }
